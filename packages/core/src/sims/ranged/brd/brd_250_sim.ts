...
export const brd250SimSpec: SimSpec<Brd250Sim, SimSettings> = {
    ...
    loadSavedSimInstance(exported: SimSettings) {
        return new Brd250Sim();
    },
    makeNewSimInstance(): Brd250Sim {
        return new Brd250Sim();
    },
    stub: "brd-250-sim",
    description: "BRD sim for 2.50 recast",
    supportedJobs: ['BRD'],
    supportedLevels: [100],
    isDefaultSim: false,
};

export interface Brd250SimResult extends SimResult {
}
...
export class Brd250Sim implements Simulation<Brd250SimResult, SimSettings, EmptyObject> {
    exportSettings() {
        return {
            ...this.settings,
        };
    ...
    async simulate(set: CharacterGearSet): Promise<Brd250SimResult> {
        ...
        const wmNoBVPotency = 4173.6;
        const wmBVPotency = 10001.8;
        const mbPotency = 8599.7;
        const apPotency = 6206.5;

        const wmNoBVSksImpactedPotency = 533;
        const wmBVSksImpactedPotency = 1374;
        const mbSksImpactedPotency = 1517.5;
        const apSksImpactedPotency = 1467;
